from mongoengine import MultipleObjectsReturned, DoesNotExist, ValidationError, NotUniqueError
...
def select_general(cls):
    ...
    print(f'\nSelecting for a {str(cls)[8:].split(".")[0]}:')
    ...
    while True:
        ...
        for column in [col[0] for col in index_info[chosen_index]['key']]:
            ...
            if '.' in column:
                document, attribute_name = column.split('.')
                attribute = getattr(cls, document)
            else:
            ...
            elif type(attribute).__name__ == "EmbeddedDocumentField":
                filters[f'{document}__{attribute_name}'] = input(f'search for {document}.{attribute_name} = --> ')
        ...
        try:
            return cls.objects(**filters).get()
        except (MultipleObjectsReturned, DoesNotExist, ValueError, InvalidId, ValidationError):
...
def validate_past_or_present(value: datetime.date) -> None:
    if value > datetime.date.today():
        raise ValidationError("Application date must be in the past or present.")


def print_dup_key_error(error: NotUniqueError):
    if not isinstance(error, NotUniqueError):
        print('Incorrect error type.')
        return
    e: str = str(error)
    print(f'Operation failed because of duplicate key already present:\n\t{e[e.find("{"):e.find("}")+1]}')
